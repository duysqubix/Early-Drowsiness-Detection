...
def blink_detector(output_textfile,input_video):
    ...
    while True:
        ...
        cv2.putText(frame, "FPS: {0:.2f}".format(fps), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0, 2))
        ...
        if (np.size(rects) != 0):
            ...
            if Q.full() and (reference_frame>15):  #to make sure the frame of interest for the EAR vector is int the mid
                ...
                print("############################ML", IF_Closed_Eyes, 'queue_cntr: {}, Queue_size: {}: Reference_frame: {}'.format(queue_cntr, Q.qsize(), reference_frame))
